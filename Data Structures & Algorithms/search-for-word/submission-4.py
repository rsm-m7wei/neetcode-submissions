class Solution:
    ...
    # 这里的功能室往上下左右扩展，看他是否和下一个字母相等，所以我们需要传入： board，word， ij（是在board里面的第几个）， curr（现在是在在word的第几个）
        ...
        #base case1 find the solution
        if curr == len(word):
            return True
        #base case2 out bound or not qualified
        if i<0 or i>=len(board) or j<0 or j>=len(board[0]) or board[i][j] !=word[curr]:
            return False
        #out of this two case we can identify current cell is whant we want and we can check surrounding cells:
        #in case of duplicate we wna to store curren cell information and replase it with empty ''
        #afterwe checked the cell we can put it back
        letter = board[i][j]
        board[i][j] = ''
        found = self.dfs(board, word, i-1,j,curr+1) or self.dfs(board, word, i+1,j,curr+1) or self.dfs(board, word, i,j-1,curr+1) or self.dfs(board, word, i,j+1,curr+1)
        board[i][j] = letter
